%%%% Comparison of EDF_LDF....LDF_EDF....Randomized schedule
...
e= 3:6;
DL= zeros(1,length(e));
% File_Name= 'LDF_EDF_10_Links_Heter';

for r= 1:length(e)

T= 10^e(r);
% lambda= [0.1*ones(1,2) 0.09 0.07*ones(1,5) 0.04*ones(1,5) 0.02*ones(1,7) 0.01];
...
% tao= [ 5*ones(1,2) 2*ones(1,5) 4*ones(1,5) 3*ones(1,9)];
% tao= 2*ones(1,L);
% tao= [4 2 3];
...
DL(r)= sum(D(:,T));
figure(r)
plot(1:L, deficit_based, 'k+', 'LineWidth',2); hold on;
...
File_Name= 'Just_LDF_10_Links_Heter';
...
DW(r)= sum(D(:,T));
figure(r)
plot(1:L, deficit_based_without, 'k+', 'LineWidth',2); hold on;
plot(1:L, p, 'LineWidth',2)
...
File_Name= 'Random_10_Links_Rand';
...
disp('Delivery ratio- linkwise');
deficit_based_rand= sum(s,2)./sum(a,2)
...
DR(r)= sum(D(:,T));
figure(r)
plot(1:L, deficit_based_rand, 'k+', 'LineWidth',2); hold on;
plot(1:L, p, 'LineWidth',2)
...
legend('LDF+EDF','LDF Without EDF','RANDOM');
xlabel('10^e');
ylabel('Deficit');
...
figure(6);
plot(1:L,p,1:L,deficit_based,1:L,deficit_based_without,1:L,deficit_based_rand,'linewidth',2);
legend('Given','LDF+EDF','LDF Without EDF','RANDOM');
grid on;
xlabel('Links');
ylabel('QOS');
