%%%% Comparison of EDF_LDF and LDF_EDF
...
DL= zeros(1,length(e));
DW= zeros(1,length(e));
% File_Name= 'LDF_THEN_EDF_Heter_Deadline_10_Links_1_000_000_Time_Slots';
...
title(['QOS Comparison of EDF+LDF for time 10^' num2str(e(r))]);
grid on;
...
for t= 1:T    
    ...
    % Links that a have a packet in the input
    prior_ind= 0;    
    if sum(prior( :,t))
        prior_ind= find(prior( :,t));               
    end 
 ...
 %%%% Deficit queue update method
    ...
    if sum(prior_ind)        
        ...
        if length(max_def_ind)>1
        
            for j= links_pack(max_def_ind,2) % The links that have same deficit
                % Now find out which are EDD
            ...
            deadlines= ED(:,t);                
            min_deadline= min(deadlines(deadlines>0));
            DEDD(find(ED( :,t)== min_deadline),t)= 1; % Largest deficit EDD packet


            DEDD_ind= 0;    
            if sum(DEDD( :,t))
                DEDD_ind= find(DEDD( :,t));               
            end
            
        else
            
            DEDD_ind= links_pack(max_def_ind,2);           
            
        end
    
        
        chosen= datasample(DEDD_ind,1); % DEDD_ind are the links that have largest deficit and same residual time packet
        s(chosen,t)= 1;
...
xlabel('Links');
ylabel('Achieved delivery ratio');
title(['QOS Comparison of LDF+EDF for time 10^' num2str(e(r))]);
grid on;
...
for i=1:n
    figure(i);
    legend('EDF+LDF','Given','LDF+EDF');
end


figure(n+1);
plot(e,DL, e,DW, 'linewidth',2); grid on;
legend('EDF+LDF','LDF+EDF');
xlabel('10^e');
ylabel('Deficit');
